...
[Contact Support >](mailto:jeremy_d._hulick@omb.eop.gov)
...
The package includes under the client/ folder the file __gulpfile.js__,  __package.json__ and __bower.json__ to install the required components to compile the source files.
...
- Open a terminal, go the package __client/__ folder, then run the command __npm install__. This command will install gulp and all project dependencies. 
...
__Note__ The edit the scripts included in base.js open the file vendor.base.json located under the client folder
...
Vendor script dependencies are managed by bower. Just run __bower install__ in folder client/ and all dependencies will be installed.

After installing all dependencies is necessary to install them in the locations the app expects them. To do that, run gulp task with the command `gulp scripts:vendor`  in the client folder.  
...
Edit the LESS files in folder __less/app__ and the file __less/bootstrap/variables.less__ to use the color you want.  
Like Bootstrap, most of the colors are based on `@brand-*` variables.

You can also edit the files in __less/theme__ folder to create your own set of color schemes. This files must be included after the __app.css__ in order to override the default color set.
